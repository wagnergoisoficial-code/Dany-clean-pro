  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right duration-500">
      ...
      <div className="flex items-center gap-4 mb-2">
        <button 
          onClick={onBack}
          className="p-2 hover:bg-surface-mid text-ink-muted transition-colors"
        >
          <ArrowLeft size={20} />
        </button>
        <div>
          <h3 className="font-bold text-2xl text-ink">{customer.name}</h3>
          <p className="text-sm text-ink-muted">Identity Profile • {isEditing ? 'Editing Mode': 'Read Only'}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        {!isEditing ? (
          <button 
            ...
            className="flex items-center gap-2 px-6 py-2 bg-ink text-white text-sm font-bold hover:bg-ink transition-all"
          >
          ...
          <>
            <button 
              ...
              className="flex items-center gap-2 px-6 py-2 bg-accent text-white text-sm font-bold hover:bg-accent-strong transition-all disabled:opacity-50"
            >
            ...
            <button 
              ...
              disabled={saveMutation.isPending}
              className="flex items-center gap-2 px-6 py-2 bg-surface-mid text-ink-muted text-sm font-bold hover:bg-surface-high transition-all"
            >
              <X size={16} /> Cancel
            </button>
          </>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {sections.map((section, idx) => (
          <div key={idx} className="bg-surface p-6 border border-rule transition-shadow">
            <div className="flex items-center gap-2 mb-6">
              <div className="w-8 h-8 bg-accent-soft text-accent flex items-center justify-center">
                <section.icon size={16} />
              </div>
              <h4 className="font-bold text-ink text-sm uppercase tracking-wider">{section.title}</h4>
            </div>

            <div className="space-y-6">
              {section.items.map((item: any, i) => (
                <div key={i} className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-label-sm text-ink-faint uppercase">
                    <item.icon size={10} /> {item.label}
                  </div>
                  {item.isStatus && isEditing ? (
                    <select 
                      className="text-sm font-medium text-ink-soft bg-surface-low border border-rule px-2 py-1 outline-none focus:ring-2 focus:ring-accent"
                      value={status}
                    ...
                    <input 
                      type="text"
                      className="text-sm font-medium text-ink-soft bg-surface-low border border-rule px-2 py-1 outline-none focus:ring-2 focus:ring-accent w-full"
                      value={item.value === 'Not provided' ? '' : item.value}
                      onChange={(e) => item.onChange(e.target.value)}
                    />
                  ) : (
                    <div className={cn(
                      "text-sm font-medium text-ink-soft truncate",
                      item.isStatus && (
                        item.value === 'active' ? "text-green-600 bg-green-50 px-2 py-0.5 w-fit border border-green-100": "text-ink-faint bg-surface-low px-2 py-0.5 w-fit"
                      )
                    )}>
                      {item.value}
                    </div>
      ...
      <div className="bg-surface-low p-8 border border-rule">
        <h4 className="text-ink mb-2 flex items-center gap-2 uppercase text-label-sm">
          <Tags size={12} className="text-ink-faint" /> Administrative Notes
        </h4>
        {isEditing ? (
          <textarea 
            className="w-full bg-surface border border-rule p-4 text-sm text-ink-soft outline-none focus:ring-2 focus:ring-accent min-h-[150px] resize-none"
            placeholder="Add administrative notes here..."
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
          />
        ) : (
          <p className="text-ink-muted text-sm whitespace-pre-wrap italic">
      ...
      {customer.leadId ? (
        <div className="bg-surface p-8 border border-rule">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-lg text-ink">Original Transition Lead</h4>
                <p className="text-xs text-ink-muted">Historical data from the initial service request.</p>
              </div>
            </div>
          </div>

          {isLoadingLead ? (
            <div className="py-8 text-center text-ink-faint italic text-sm">Loading historical lead data...</div>
          ) : !originalLead ? (
            <div className="py-8 text-center bg-surface-low border border-dashed border-rule text-ink-faint text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-1">
                <p className="text-label-sm text-ink-faint uppercase">Service Requested</p>
                <p className="text-sm font-bold text-ink-soft">{originalLead.service_type}</p>
              </div>
              <div className="space-y-1">
                <p className="text-label-sm text-ink-faint uppercase">Home Size</p>
                <p className="text-sm font-bold text-ink-soft">{originalLead.bedrooms} Bed / {originalLead.bathrooms} Bath</p>
              </div>
              <div className="space-y-1">
                <p className="text-label-sm text-ink-faint uppercase">Initial Status</p>
                <span className="px-2 py-0.5 bg-surface-mid text-ink-muted text-label-sm uppercase">
                  {originalLead.status}
                </span>
              </div>
              <div className="space-y-1">
                <p className="text-label-sm text-ink-faint uppercase">Conversion Date</p>
                <p className="text-sm font-bold text-ink-soft">{formatDate(customer.createdAt)}</p>
              </div>
              {originalLead.message && (
                <div className="col-span-full pt-4 border-t border-rule">
                  <p className="text-label-sm text-ink-faint uppercase mb-1">Initial Message/Notes</p>
                  <p className="text-xs text-ink-muted italic">"{originalLead.message}"</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-surface-low p-6 border border-rule flex items-center justify-center gap-3 text-ink-faint">
          <Info size={16} />
      ...
      <div className="bg-surface p-8 border border-rule">
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 bg-surface-mid text-ink-muted flex items-center justify-center">
            <HistoryIcon size={20} />
          </div>
          <div>
            <h4 className="font-bold text-lg text-ink">Customer Timeline</h4>
            <p className="text-xs text-ink-muted">Operational history and major events.</p>
          </div>
        </div>
        <CustomerTimeline customerId={customer.id} />
      </div>

      <div className="bg-accent p-6 text-white flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <p className="font-bold text-lg">Operational History</p>
          <p className="text-white/80 text-sm">Total cleanings completed: <span className="font-bold underline">{customer.totalBookings || 0}</span></p>
        </div>
        <div className="flex items-center gap-2 text-label-sm uppercase bg-white/10 px-4 py-2 backdrop-blur-sm border border-white/10">
